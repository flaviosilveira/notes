...
The way that we organize and aggregate data.
...
Array is a contiguous data structure. That means that arrays are stored in blocks of memory that are right beside each other with no gaps. So, retrieving values will be easier.
...
We can say that append has a non constant space complexity, but since some operations do not increase space and other do, in average append operations take constant space. We call that amortized constant space complexity.
