...
- Constants can never be changed or undefined

- class_implements function receives an object or a class name and returns an array of Interface names

- class_exists function determines whether a class exists or not

- A zero as string in a variable, will return false in a if statement

- $ in PHP, comes from Perl. In Perl there are other signs to start a variable, they are called sigils. 
Perl got them from Shell. The main use of that can be the interpolation of strings. Without it, you will need a special syntax to do it, or a function, etc.
